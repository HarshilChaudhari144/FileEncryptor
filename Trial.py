    ...
    while True:
        ...
        choice = questionary.select(
            ...
            choices=[
                ...
                f"5. {'🗃️ ' if icons else ''}Encrypt Folder (as Archive)",
                f"6. {'🗃️ ' if icons else ''}Decrypt Archive",
                f"7. {'⚙️  ' if icons else ''}Settings",
                f"8. {'📘 ' if icons else ''}Help & Usage Guide",
                f"9. {'📑 ' if icons else ''}View Recent Activities",
                f"10. {'❌ ' if icons else ''}Exit",
            ],
            style=custom_style
        ).ask().split(".")[0]
